...
class HeaderItemDecoration(
    ...
    private fun getTopChildView(recyclerView: RecyclerView) = recyclerView.findChildViewUnder(
    ...
    private fun drawMovedHeader(
        c: Canvas,
        contactedTopHeader: View,
        contactedBottomHeader: View,
        paddingTop: Int
    ) {
        c.save()
        ...
        recyclerView.addOnItemTouchListener(object : RecyclerView.SimpleOnItemTouchListener() {
            override fun onInterceptTouchEvent(
                ...
                return if (motionEvent.action == MotionEvent.ACTION_DOWN) {
                    val hasClickedOnHeaderArea =
                        motionEvent.y <= currentHeaderToShow?.second?.itemView?.bottom ?: 0
